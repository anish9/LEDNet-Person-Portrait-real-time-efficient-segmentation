...
multi.compile(optimizer=Nadam(1e-4),loss="sparse_categorical_crossentropy", metrics=["accuracy"])
...
multi.fit_generator(train_gens,steps_per_epoch=int(train_steps),validation_data=val_gens,validation_steps=val_steps,
                    epochs=198,callbacks=callback)

multi.save("model_weight.h5")
